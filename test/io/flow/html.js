...
test('html', async function (t) {
  ...
  await t.test('2 (comment)', function () {
    assert.equal(
      micromark('<!-- Foo\n\nbar\n   baz -->\nokay', unsafe),
      '<!-- Foo\n\nbar\n   baz -->\n<p>okay</p>',
  ...
  await t.test('3 (instruction)', function () {
    assert.equal(
      micromark("<?php\n\n  echo '>';\n\n?>\nokay", unsafe),
      "<?php\n\n  echo '>';\n\n?>\n<p>okay</p>",
  ...
  await t.test('4 (declaration)', function () {
    assert.equal(
  ...
  await t.test('7 (complete)', function () {
    assert.equal(
