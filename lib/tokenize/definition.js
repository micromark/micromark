...
function resolveDefinition(events, context) {
  ...
  while (++index < length) {
    ...
    if (events[index][0] === 'enter') {
      if (
        token.type === types.definitionLabelString ||
        token.type === types.definitionDestinationString ||
        token.type === types.definitionTitleString
      ) {
        token.contentType = 'string'
      }

      if (token.type === types.definitionLabelString) {
        identifier = normalizeIdentifier(context.sliceSerialize(token))
