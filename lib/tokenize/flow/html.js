...
function tokenizeHtml(effects, ok, nok) {
  ...
  function open(code) {
    ...
    if (code === codes.questionMark) {
      kind = constants.kindFlowHtmlInstruction
      effects.consume(code)
  ...
  function declarationStart(code) {
    if (code === codes.dash) {
      kind = constants.kindFlowHtmlComment
      effects.consume(code)
      return commentOpenInside
    }

    if (code === codes.leftSquareBracket) {
      kind = constants.kindFlowHtmlCdata
    ...
    if (asciiAlpha(code)) {
      kind = constants.kindFlowHtmlDeclaration
      effects.consume(code)
      return continuationDeclarationInside
    }
  ...
  function tagName(code) {
    ...
    if (
      ...
      if (raw && endTag === false && selfClosing === false) {
        kind = constants.kindFlowHtmlRaw
        return continuation
      }

      if (basic) {
        kind = constants.kindFlowHtmlBasic
      ...
      // Do not support `kind: 7` when interrupting.
      if (self.check) {
        return nok(code)
      }

      kind = constants.kindFlowHtmlComplete
  ...
  function continuation(code) {
    ...
    if (kind === constants.kindFlowHtmlRaw && code === codes.lessThan) {
      effects.consume(code)
      return continuationRawTagOpen
    }

    if (kind === constants.kindFlowHtmlComment && code === codes.dash) {
      effects.consume(code)
