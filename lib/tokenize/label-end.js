  ...
  while (index--) {
    ...
    if (event[0] === 'enter') {
      ...
      // Find where the link or image starts.
        ...
        if (
          token.type === types.resourceTitleString ||
          token.type === types.resourceDestinationString ||
          token.type === types.referenceString
        ) {
...
function tokenizeFullReference(effects, ok, nok) {
  ...
  function referenceData(code) {
    ...
    if (code === codes.rightSquareBracket) {
      ...
      if (defined.indexOf(normalizeIdentifier(sliceSerialize(token))) < 0) {
        return nok(code)
      }
