...
function tokenizeCharacterReference(effects, ok, nok) {
  ...
  function named(code) {
    ...
    if (
      buffer.length !== constants.maxCharacterReferenceNamedSize &&
  ...
  function hexadecimal(code) {
    ...
    if (
      size !== constants.maxCharacterReferenceHexadecimalSize &&
      asciiHexDigit(code)
    ) {
      size++
      effects.consume(code)
  ...
  function decimal(code) {
    ...
    if (
      size !== constants.maxCharacterReferenceDecimalSize &&
      asciiDigit(code)
    ) {
      size++
      effects.consume(code)
