...
function resolveTo(events, helpers) {
  ...
  if (kind === 'resource') {
    ...
    // Either shortcut (`[text]`) or collaped (`[text][]`) or full
    // (`[text][label]`).
    origin = referenceIndex === undefined ? [] : events.slice(referenceIndex)

    // For shortcut and collaped:
    if (referenceIndex === undefined || referenceDataIndex === undefined) {
      identifierEvents = events.slice(openIndex + 2, closeIndex - 1)

      // Empty, such as `[]` or `[][]`.
      if (identifierEvents.length === 0) {
        id = ''
      } else {
        id = identifierEvents[0][2].sliceSerialize({
          start: clone(identifierEvents[0][1].start),
          end: clone(identifierEvents[identifierEvents.length - 1][1].end)
        })
      }
    }
    // For full:
      ...
      id = event[2].sliceSerialize(event[1])
    }
  }
