...
function resolveTo(events, helpers) {
  ...
  if (kind === types.resource) {
    ...
    if (destinationIndex) {
      result = events
        ...
        .concat(tokenizeEvent(events[destinationIndex], core.plainText))

      if (titleIndex) {
        result = result.concat(
          events.slice(destinationIndex, titleIndex),
          tokenizeEvent(events[titleIndex], core.plainText),
  ...
  // Reference.
    ...
    if (referenceTextIndex) {
      result = [].concat(
        ...
        tokenizeEvent(events[referenceTextIndex], core.plainText),
  ...
  return [].concat(
    ...
    resolveAllAttention(events.slice(openIndex + 1, closeIndex), helpers),
    [['exit', text, helpers], events[closeIndex], ['exit', label, helpers]],
    result,
    [['exit', group, helpers]]
  )
}
...
function tokenizeResource(effects, ok, nok) {
  ...
  function titleEscape(code) {
    if (code === expectedTitleEndMarker || code === codes.backslash) {
      effects.consume(code)
      return title
    }
  ...
  function titleEnd(code) {
    assert(code === expectedTitleEndMarker, 'expected proper title end marker')
    effects.enter(types.resourceTitleMarker)
    effects.consume(code)
    effects.exit(types.resourceTitleMarker)
...
function tokenizeReference(effects, ok, nok) {
  ...
  function referenceData(code) {
    ...
    if (code === codes.rightSquareBracket) {
      ...
      return content ? end(code) : nok(code)
    }
