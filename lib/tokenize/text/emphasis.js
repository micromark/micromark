...
function resolveAllEmphasis(events) {
  ...
  // As `stackBottom` is always `null`, because we don’t support passing it,
  // this can be optimized by tracking the first sequence instead and dropping
  // the whole loop.
  closer = lastSequence
  ...
  while (lastSequence !== null && lastSequence !== stackBottom) {
    unlink(lastSequence)
  }
