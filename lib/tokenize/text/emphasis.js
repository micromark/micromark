...
function resolveAllEmphasis(events) {
  ...
  events.forEach(([name, token], index) => {
    var previous
    var events
  ...
  for (var i = 0; i < 3; i++) {
    bottom = {}
    bottom[asterisk] = stackBottom
    bottom[underscore] = stackBottom
...
function tokenizeEmphasis(effects, ok, nok) {
  ...
  function start(code) {
    ...
    if (code !== asterisk && code !== underscore) return nok
...
function classify(code) {
  if (code !== code || code === lineFeed || unicodeWhitespace(code)) {
    return 'whitespace'
  }
