...
function html() {
  ...
  function onexitlabeltext(token, helpers) {
    media.labelId = normalizeIdentifier(helpers.sliceSerialize(token))
  }
  ...
  function onexitreferencetext(token, helpers) {
    media.referenceId = normalizeIdentifier(helpers.sliceSerialize(token))
  }
  ...
  function onexitdefinitionlabeldata(token, helpers) {
  ...
  function onexitatxheadingsequence(token, helpers) {
    if (headingRank) return
    headingRank = helpers.sliceSerialize(token).length
  ...
  function onexitsetextheadinglinesequence(token, helpers) {
    headingRank =
      helpers.sliceSerialize(token).charCodeAt(0) === codes.equalsTo ? 1 : 2
  }
  ...
  function onexitdata(token, helpers) {
    send(encode(helpers.sliceSerialize(token)))
  }

  function onexitlineending(token, helpers) {
  ...
  function onexitcodeflowdata(token, helpers) {
  ...
  function onexithtmlflow(token, helpers) {
    send(unsafe(helpers.sliceSerialize(token)))
  }

  // To do: safe by default?
  // To do: should EOLs / linePrefix be separate tokens?
  function onexithtmlspan(token, helpers) {
    send(unsafe(helpers.sliceSerialize(token).replace(/([\r\n]) +/g, '$1')))
  }
  ...
  function onexitcodespandata(token, helpers) {
    send(
      encode(helpers.sliceSerialize(token).replace(/\r?\n|\r/g, values.space))
    )
  }
  ...
  function onexitcharacterescapevalue(token, helpers) {
    send(encode(helpers.sliceSerialize(token)))
  }
  ...
  function onexitcharacterreferencevalue(token, helpers) {
    var data = helpers.sliceSerialize(token)
    var value
  ...
  function onexitautolinkuri(token, helpers, prefix) {
    var uri = encode(helpers.sliceSerialize(token))
    send(tag('<a href="' + (prefix || '') + normalizeUri(uri) + '">'))
    send(uri)
    send(tag('</a>'))
  }

  function onexitautolinkemail(token, helpers) {
    onexitautolinkuri(token, helpers, 'mailto:')
  }
  ...
  function unsafe(value) {
    /* istanbul ignore next - to do: unsafe/safe toggle. */
