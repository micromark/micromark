  ...
  while (++index < events.length) {
    ...
    if (event[0] === 'enter') {
      ...
      if (subevents) {
        ...
        splice(events, index, 2, subevents)
    ...
    // If this is an exit of a container:
      ...
      if (lineIndex) {
        ...
        splice(
          ...
          lineIndex,
          index - lineIndex + 1,
          [event].concat(events.slice(lineIndex, index))
        )
        index = lineIndex
      }
    }
...
function unravelLinkedTokensImpl(token, context) {
...
function unravelLinkedTokens(token, context) {
  while (token) {
    ...
    unravelLinkedTokensImpl(token, context)
