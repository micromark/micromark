...
function createTokenizer(parser, initializer, from) {
  ...
  var state = initializer.call(
    {parser: parser, previous: previousCode, check: false, queue: eventQueue},
    effects
  )
  ...
  function write(value) {
    ...
    while (++index < length) {
      eventQueue = attachedResolveAlls[index](eventQueue, context)
    }
  ...
  function enter(type) {
    ...
    eventQueue.push(['enter', token, context])
  ...
  function exit(type) {
    ...
    eventQueue.push(['exit', token, context])
  ...
  function createConstructAttempt(constructs, returnState, bogusState) {
    ...
    function ok() {
      ...
      eventQueue = resolveTo(
        info.queue.concat(resolve(eventQueue, context)),
        context
      )
  ...
  function isConstruct(construct, returnState, bogusState) {
    ...
    return construct.tokenize.call(
      {parser: parser, previous: previousCode, check: true, queue: info.queue},
      effects,
      ok,
      nok
    )
  ...
  function createConstruct(construct, returnState) {
    ...
    return construct.tokenize.call(this, effects, ok)

    function ok() {
      // istanbul ignore next - this function is currently only used for
      // constructs w/ `resolve`.
      var resolve = construct.resolve || identity
      var resolveTo = construct.resolveTo || identity

      eventQueue = resolveTo(
        startEventQueue.concat(resolve(eventQueue, context)),
        context
      )
