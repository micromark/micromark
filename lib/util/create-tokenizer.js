...
function createTokenizer(parser, initialize, from) {
  ...
  function onsuccessfultokenize(construct, info) {
    // Use results.
    addResult(construct, info.events)
  }
    ...
    function handleOneOrMoreConstructs(constructs, returnState, bogusState) {
      ...
      function handleConstruct(construct) {
        ...
        return construct.tokenize.call(
          assign({}, context, props, {previous: last, events: info.events}),
          effects,
          ok,
          nok
        )
  ...
  function addResult(construct, before) {
    ...
    if (construct.resolve) {
      events = construct.resolve(events, context)
    }
  ...
  function store() {
    ...
    // Perf to do: can we ignore this concat, and do `events.length = x`?
    ...
    return {restore: restore, events: startEvents}
