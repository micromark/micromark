...
function compileHtml(options) {
  ...
  var fencedCodeInside
  var flowCodeSeenData
  var slurpAllLineEndings
  var slurpOneLineEnding
        ...
        handler[result[index][1].type].call(
          {
            ...
            buffer: buffer,
            resume: resume,
            setSlurpOneLineEnding: setSlurpOneLineEnding,
            setIgnoreEncode: setIgnoreEncode
          },
          result[index][1]
        )
      }
  ...
  function setSlurpOneLineEnding(value) {
    slurpOneLineEnding = value
  }

  function setIgnoreEncode(value) {
    ignoreEncode = value
  }
  ...
  function onexitlistitem() {
    if (lastWasTag && !slurpAllLineEndings) {
      lineEndingIfNeeded()
    }

    tag('</li>')
    slurpAllLineEndings = undefined
  }
  ...
  function onexitcodefencedfence() {
    ...
    if (fencedCodeInside) return
    tag('>')
    fencedCodeInside = true
    setSlurpOneLineEnding(true)
  }
  ...
  function onexitflowcode() {
    ...
    if (flowCodeSeenData) lineEndingIfNeeded()
    tag('</code></pre>')
    fencedCodeInside = undefined
    flowCodeSeenData = undefined
    setSlurpOneLineEnding()
  }
  ...
  function onexitlineending(token) {
    if (slurpAllLineEndings) {
      return
    }

    if (slurpOneLineEnding) {
      setSlurpOneLineEnding()
      return
    }
