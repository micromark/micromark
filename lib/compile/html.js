...
/**
 * @typedef Context HTML compiler context
 * @property {Options} options Configuration passed by the user.
 * @property {(key: string, value?: unknown) => void} setData Set data into the key-value store.
 * @property {<K extends string>(key: K) => Data[K]} getData Get data from the key-value store.
 * @property {() => void} lineEndingIfNeeded Output an extra line ending if the previous value wasn’t EOF/EOL.
 * @property {(value: string) => string} encode Make a value safe for injection in HTML (except w/ `ignoreEncode`).
 * @property {() => void} buffer Capture some of the output data.
 * @property {() => string} resume Stop capturing and access the output data.
 * @property {(value: string) => void} raw Output raw data.
 * @property {(value: string) => void} tag Output (parts of) HTML tags.
 * @property {Tokenizer['sliceSerialize']} sliceSerialize
 */

/**
 * @typedef {(events: Event[]) => string} Compile Serialize micromark events as HTML
 *
 * @typedef {(this: Context, token: Token) => void} Handle Handle one token
 * @typedef {(this: Omit<Context, 'sliceSerialize'>) => void} DocumentHandle Handle the whole
 ...
 * @typedef {Partial<NormalizedHtmlExtension>} HtmlExtension An HTML extension changes how markdown tokens are serialized.
 */
...
import {combineHtmlExtensions} from '../util/combine-html-extensions.js'
import {chunkedPush} from '../util/chunked-push.js'
import {normalizeIdentifier} from '../util/normalize-identifier.js'
import {normalizeUri} from '../util/normalize-uri.js'
import {safeFromInt} from '../util/safe-from-int.js'
 ...
 * @returns {Compile}
 */
export function compileHtml(options = {}) {
  /**
   ...
   */
  // @ts-expect-error because `defaultHandlers` is normalized, `handlers` will
  // be too.
