...
var codeFenced = require('../tokenize/flow/code-fenced')
var html = require('../tokenize/flow/html')
var codeIndented = require('../tokenize/flow/code-indented')
...
function flow(effects) {
  ...
  initialFlowHooks[codes.ht] = codeIndented
  initialFlowHooks[codes.space] = codeIndented
  ...
  flowHooks[codes.graveAccent] = codeFenced
  flowHooks[codes.tilde] = codeFenced
  ...
  function tokenizeLookaheadConstruct(effects, ok, nok) {
    ...
    function maybeHtml() {
      return effects.isConstruct(html, ok, maybeCodeFenced)
    }
  ...
  function resolveContent(events) {
    return [].concat([events[0]], tokenizeEvent(events[0], core.content), [
      events[1]
    ])
  }
}
