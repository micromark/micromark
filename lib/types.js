 ...
 * Like a tokenizer, but without `ok` or `nok`.
 *
 * @typedef {Construct & {tokenize: Initialize}} Initializer
 */

/**
 * Several constructs, mapping from their initial codes.
 *
 * @typedef {Record<string, undefined|Construct|Construct[]>} ConstructObject
 */
 ...
 * @property {(token: Pick<Token, 'start'|'end'>) => string} sliceSerialize Get the original string that spans a token.
 * @property {() => Point} now
...
/**
 * @typedef _SyntaxExtensionFields
 ...
 *
 * @typedef {Record<string, Record<string, unknown>> & Partial<_SyntaxExtensionFields>} Extension
 */
...
/**
 * @typedef _NormalizedSyntaxExtensionFields
...
/**
 * @typedef {Record<string, Record<string, unknown[]>> & _NormalizedSyntaxExtensionFields} FullNormalizedExtension
 * @typedef {Partial<FullNormalizedExtension>} NormalizedExtension
 */

/**
 * @typedef {import('./compile/html.js').Handle} Handle
 * @typedef {import('./compile/html.js').DocumentHandle} DocumentHandle
 * @typedef {import('./compile/html.js').HtmlExtension} HtmlExtension
 */
